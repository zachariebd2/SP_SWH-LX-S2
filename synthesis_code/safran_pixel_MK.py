...
TMP_PATH = f"/home/ad/barrouz/zacharie/TIMESERIES_PROJECT/SYNTHESIS/ANALYSIS/SAFRAN/TMP/pixel_{ele}_{mtn}_{N}.tif"
...
for minnobs in MIN_NOBS:
    for i,metric in enumerate(metric_list):
        ...
        for year in period:
            ...
            scd_list=[]
            for tile in tile_list:
                swh_landsat_path = glob.glob(op.join(synthese_model,mtn,tile,"*",f"LIS_S2L8-SNOW-SMOD_{tile}_{str(year)}*.tif"),recursive=True)
        ...
        for year in theia_period:
            ...
            scd_list=[]
            for tile in tile_list:
                swh_landsat_path = glob.glob(op.join(THEIA_path,mtn,tile,f"*_{str(year)}0*",f"*SM*.tif"),recursive=True)
                nobs_path = glob.glob(op.join(THEIA_path,mtn,tile,f"*_{str(year)}0*",f"*NOB*.tif"),recursive=True)
                scd_path = glob.glob(op.join(THEIA_path,mtn,tile,f"*_{str(year)}0*",f"*SCD*.tif"),recursive=True)
                if len(swh_landsat_path) == 1:
                    raster_list.extend(swh_landsat_path)
                    nobs_list.extend(nobs_path)
                    scd_list.extend(scd_path)
...
df_path = f"/home/ad/barrouz/zacharie/TIMESERIES_PROJECT/SYNTHESIS/ANALYSIS/SAFRAN/DATAFRAMES/safran_pixel_{mtn}_{N}_{ele}.pkl"
